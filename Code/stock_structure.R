...
all.catch <- all.catch %>% 
  mutate(avg.shark.wt = catch.weight/catch.numbers)
...
ggplot(all.catch[!is.na(all.catch$catch.numbers) & all.catch$Year >=2013,], 
       aes(Year, avg.shark.wt*1000, color=FMP)) +
  geom_point(size=2) +
  geom_line() +
  #facet_wrap(~FMP, ncol=1, scales="free_y") +
  labs(x="\nYear", y="Average shark weight (kg)\n") + 
  scale_color_manual(values = c("#374E55FF", "#DF8F44FF", "#B24745FF")) +
  scale_y_continuous(labels = scales::comma) +
